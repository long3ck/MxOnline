...
class ModifyPwdView(View):
    ...
    def post(self,request):
        ...
        if modify_form.is_valid():
            ...
            email = request.POST.get("email","")
            return render(request,"password_reset.html",{"email":email,"modify_from":modify_form})
    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=user_name):
                return render(request,'register.html',{"register_form":register_form,"msg":"您注册的用户已存在！！！"})
            pass_word = request.POST.get("password","")
...
class SendEmailCodeView(LoginRequiredMixin,View):
    ...
    def get(self,request):
        ...
        if UserProfile.objects.filter(email=email):
            return HttpResponse(json.dumps({'email':'该邮箱地址已存在'}), content_type='application/json')
